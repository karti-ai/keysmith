      ...
      <div className="transaction-layout">
        <div className="transaction-main">
          ...
          <section className="transaction-panel evidence-flow"><h2>Deterministic evidence flow</h2>{[
            ["Read live state", "Captured the connected USB inspection and keymap."],
            ...
            ["Stop for confirmation", "No web mutation path exists; installed v0.3 still requires its physical chord and an attended host commit."],
          ...
          <div className="transaction-warning"><ShieldAlert /><span><strong>Web and agent paths never apply mutations.</strong> {blocked.length ? `${blocked.length} draft entries are explicitly non-executable because rollback is incomplete.` : "Eligible entries match installed v0.3, but execution remains a separate attended terminal workflow with physical confirmation."}</span></div>
        </div>
        <aside className="preview-summary">
          ...
          <div className="next-step"><h3>What happens next</h3><p>The JSON can be compiled into one plan-bound operation at a time by the attended CLI. Every operation requires the physical chord and exact host commit.</p></div>
          <div className="compatibility-warning"><ShieldAlert /><span><strong>v0.3 installed safely</strong>The stable VIA marker preserved the complete configuration. The live gate relocks by default; this browser still cannot write.</span></div>
